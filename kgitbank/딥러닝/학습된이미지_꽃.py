...
def extract_features(directory, sample_count):
    #block5_pool (MaxPooling2D)   (None, 4, 4, 512)    - 현재 마지막층      
    #받은 데이터 개수 만큼 0으로 채워진 4 by 4 by 512 배열 만들기 
    features = np.zeros(shape=(sample_count, 4, 4, 512)) #0 으로 채워진 4차원 배열을 만든다. 
    #라벨 - 정답 만들기                                  
    labels = np.zeros(shape=(sample_count))   #라벨 
    ...
    i = 0
    for inputs_batch, labels_batch in generator:  #이미지, 라벨 
